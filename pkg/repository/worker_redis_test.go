...
func TestReconcileWorkerCapacityRepairsUnacknowledgedStopWithoutOvercreditingReplacement(t *testing.T) {
	rdb, err := NewRedisClientForTest()
	assert.NotNil(t, rdb)
	assert.Nil(t, err)

	workerRepo := NewWorkerRedisRepositoryForTest(rdb)
	containerRepo := NewContainerRedisRepository(rdb)
	worker := &types.Worker{
		Id:            "worker-stale-stop",
		Status:        types.WorkerStatusAvailable,
		TotalCpu:      1000,
		TotalMemory:   125,
		TotalGpuCount: 1,
	...
	old := &types.ContainerRequest{
		ContainerId: "container-stale-stop",
	...
	assert.Nil(t, containerRepo.SetContainerState(old.ContainerId, &types.ContainerState{
		...
		Memory:      old.Memory,
		Gpu:         old.Gpu,
		GpuCount:    old.GpuCount,
	}))
	assert.Nil(t, workerRepo.ScheduleContainerRequest(worker, old))
	delivered, err := workerRepo.GetNextContainerRequests(worker.Id, 1)
	assert.Nil(t, err)
	assert.Equal(t, 1, len(delivered))
	assert.Nil(t, workerRepo.AddContainerToWorker(worker.Id, old.ContainerId, delivered[0].DeliveryToken))
	assert.Nil(t, containerRepo.UpdateContainerStatus(old.ContainerId, types.ContainerStatusStopping, types.ContainerStateTtlSWhilePending))
	assert.Nil(t, workerRepo.ReconcileWorkerCapacity(worker.Id))

	repaired, err := workerRepo.GetWorkerById(worker.Id)
	assert.Nil(t, err)
	assert.Equal(t, int64(1000), repaired.FreeCpu)
	assert.Equal(t, int64(125), repaired.FreeMemory)
	assert.Equal(t, uint32(1), repaired.FreeGpuCount)

	replacement := &types.ContainerRequest{
	...
	assert.Nil(t, containerRepo.SetContainerState(replacement.ContainerId, &types.ContainerState{
		ContainerId: replacement.ContainerId,
		WorkspaceId: replacement.WorkspaceId,
		StubId:      replacement.StubId,
		Status:      types.ContainerStatusPending,
		Cpu:         replacement.Cpu,
		Memory:      replacement.Memory,
		Gpu:         replacement.Gpu,
		GpuCount:    replacement.GpuCount,
	}))
	assert.Nil(t, workerRepo.ScheduleContainerRequest(repaired, replacement))
	delivered, err = workerRepo.GetNextContainerRequests(worker.Id, 1)
	assert.Nil(t, err)
	assert.Equal(t, 1, len(delivered))
	assert.Nil(t, workerRepo.AddContainerToWorker(worker.Id, replacement.ContainerId, delivered[0].DeliveryToken))
	assert.Nil(t, workerRepo.ReconcileWorkerCapacity(worker.Id))

	occupied, err := workerRepo.GetWorkerById(worker.Id)
	assert.Nil(t, err)
	assert.Equal(t, int64(0), occupied.FreeCpu)
	assert.Equal(t, int64(0), occupied.FreeMemory)
	assert.Equal(t, uint32(0), occupied.FreeGpuCount)
}

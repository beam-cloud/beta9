...
// is final once journaled, so a duplicate completion writes nothing; only a
// pre-consolidation record is rewritten. Written charges stay in the
// pending index until CompleteAccounting.
...
// score. Returns 1 when the charge was written, 0 when a charge already existed.
...
if existing then
	-- Only a pre-consolidation record (no status) may be rewritten.
	if cjson.decode(existing).status ~= nil then return 0 end

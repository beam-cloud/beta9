...
func init() {
	goose.AddMigrationContext(upEnsureCheckpointCacheMetadata, downEnsureCheckpointCacheMetadata)
}

func upEnsureCheckpointCacheMetadata(ctx context.Context, tx *sql.Tx) error {
	_, err := tx.Exec(`
...
func downEnsureCheckpointCacheMetadata(ctx context.Context, tx *sql.Tx) error {
	_, err := tx.Exec(`

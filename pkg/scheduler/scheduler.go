...
const (
	...
	checkpointHandoffRetryDelay    time.Duration = time.Second
)
...
func (s *Scheduler) processRequest(request *types.ContainerRequest, workers []*types.Worker) {
	if !s.checkpointSchedulingReady(request) {
...
func (s *Scheduler) checkpointSchedulingReady(request *types.ContainerRequest) bool {

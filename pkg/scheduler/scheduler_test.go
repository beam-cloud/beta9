...
func TestSchedulingPerformance_ParameterComparison(t *testing.T) {
	...
	for _, cfg := range configs {
		t.Run(cfg.name, func(t *testing.T) {
			// Skip this test for now - wave-based processing changes the performance characteristics
			// The new scheduler_batch_optimization_test.go has more accurate benchmarks
			t.Skip("Deprecated - use scheduler_batch_optimization_test.go for performance testing")
		})
	}
}

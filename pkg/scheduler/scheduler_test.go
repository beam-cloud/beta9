...
func TestCheckpointSchedulingWaitsWhileStoppingWithoutCheckpoint(t *testing.T) {
	...
	assert.False(t, scheduler.checkpointSchedulingReady(request))
...
func TestCheckpointSchedulingSkipsHandoffCheckWithCheckpoint(t *testing.T) {
	scheduler, err := NewSchedulerForTest()
	assert.NoError(t, err)
	backend := &checkpointBackendRepoForTest{BackendRepository: scheduler.backendRepo}
	scheduler.backendRepo = backend

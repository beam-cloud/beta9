...
func TestExplicitCheckpointOnlyProvisionsRestoreCapableRuntime(t *testing.T) {
	scheduler, err := NewSchedulerForTest()
	assert.Nil(t, err)
	scheduler.workerPoolManager = NewWorkerPoolManager()

	runc := &LocalWorkerPoolControllerForTest{
		name:             "rtx5090-runc",
		containerRuntime: types.ContainerRuntimeRunc.String(),
	}
	gvisor := &LocalWorkerPoolControllerForTest{
		name:             "rtx5090-gvisor",
		containerRuntime: types.ContainerRuntimeGvisor.String(),
	}
	scheduler.workerPoolManager.SetPool(
		runc.name,
		types.WorkerPoolConfig{GPUType: "RTX5090", ContainerRuntime: types.ContainerRuntimeRunc.String()},
		runc,
	)
	scheduler.workerPoolManager.SetPool(
		gvisor.name,
		types.WorkerPoolConfig{GPUType: "RTX5090", ContainerRuntime: types.ContainerRuntimeGvisor.String()},
		gvisor,
	)
	...
	assert.Nil(t, err)
	assert.Equal(t, []WorkerPoolController{gvisor}, controllers)
}

func TestExplicitCheckpointNeverCallsRuncController(t *testing.T) {
	scheduler, err := NewSchedulerForTest()
	assert.Nil(t, err)
	scheduler.workerPoolManager = NewWorkerPoolManager()
	...
	scheduler.workerPoolManager.SetPool(
		runc.name,
		types.WorkerPoolConfig{GPUType: "RTX4090", ContainerRuntime: types.ContainerRuntimeRunc.String()},
		runc,
	)

	request := &types.ContainerRequest{
		ContainerId: uuid.New().String(),
		GpuRequest:  []string{"RTX4090"},
		GpuCount:    1,
		Checkpoint:  &types.Checkpoint{CheckpointId: "checkpoint-1"},
		Timestamp:   time.Now(),
	}
	for range 100 {
		newSchedulingAttempt(scheduler, request.Clone(), nil).provisionWorker()
	}
	time.Sleep(2 * requestProcessingInterval)
	assert.Equal(t, 0, runc.AddWorkerCallCount())
}

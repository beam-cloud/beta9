...
func NewLocalKubernetesWorkerPoolController(ctx context.Context, config types.AppConfig, workerPoolName string, workerRepo repository.WorkerRepository, providerRepo repository.ProviderRepository) (WorkerPoolController, error) {
	...
	if err != nil {
		common.Logger.Infof(wpc.ctx, "<pool %s> unable to monitor pool size: %+v", wpc.name, err)
	}
...
func (wpc *LocalKubernetesWorkerPoolController) deleteStalePendingWorkerJobs() {
	...
	for range ticker.C {
		...
		if err != nil {
			common.Logger.Infof(wpc.ctx, "Failed to list jobs for controller <%s>: %v", wpc.name, err)
			continue
		}
		...
		for _, job := range jobs.Items {
			...
			if err != nil {
				common.Logger.Infof(wpc.ctx, "Failed to list pods for job <%v>: %v", job.Name, err)
				continue
			}
				...
				if duration >= maxAge {
					...
					if workerId, ok := pod.Labels[Beta9WorkerLabelIDKey]; ok {
						if err := wpc.workerRepo.RemoveWorker(&types.Worker{Id: workerId}); err != nil {
							common.Logger.Infof(wpc.ctx, "Failed to delete pending worker <%s> from repo: %v", workerId, err)
						}
					}
					...
					if err := wpc.kubeClient.BatchV1().Jobs(namespace).Delete(ctx, job.Name, metav1.DeleteOptions{
						...
						common.Logger.Infof(wpc.ctx, "Failed to delete pending worker job <%s>: %v", job.Name, err)
					}

					common.Logger.Infof(wpc.ctx, "Deleted worker <%s> due to exceeding age limit of <%v>", job.Name, maxAge)
				}
			}
		}
	}

...
func (b *Builder) Build(ctx context.Context, opts *BuildOpts, outputChan chan common.OutputMsg) error {
	...
	common.Logger.Infof(ctx, "container <%v> building with options: %s\n", containerId, opts)
	startTime := time.Now()
	...
	for _, cmd := range opts.Commands {
		...
		if r, err := client.Exec(containerId, cmd); err != nil || !r.Ok {
			common.Logger.Infof(ctx, "failed to execute command for container <%v>: \"%v\" - %v\n", containerId, cmd, err)
	...
	common.Logger.Infof(ctx, "container <%v> build took %v\n", containerId, time.Since(startTime))

...
// webSocketHandshakeHeaders are generated by the dialer and cannot be passed
// through from the incoming request.
...
// forwardedWebSocketHeaders preserves end-to-end request headers while leaving
// the WebSocket handshake headers to the backend dialer.
func forwardedWebSocketHeaders(request *http.Request) http.Header {
	forwarded := request.Header.Clone()
	// Host is stored on Request rather than in Header. The backend needs the
	// public host for origin checks instead of the proxy's internal destination.

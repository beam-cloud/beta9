...
func (s *GenericPodService) run(ctx context.Context, authInfo *auth.AuthInfo, stub *types.StubWithRelated, snapshotId *string, checkpoint *types.Checkpoint) (string, error) {
...
func (s *GenericPodService) CreatePod(ctx context.Context, in *pb.CreatePodRequest) (*pb.CreatePodResponse, error) {
	...
	containerId, err := s.run(ctx, authInfo, stub, in.SnapshotId, checkpoint)
	if err != nil {
		return &pb.CreatePodResponse{
			Ok: false,
		}, nil

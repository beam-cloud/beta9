...
func (i *appInstance) startContainers(containersToRun int) error {
	...
	for c := 0; c < containersToRun; c++ {
		...
		i.Rdb.SetEx(
			context.Background(),
			Keys.appKeepWarmLock(i.Workspace.Name, i.Stub.ExternalId, containerId),
			1,
...
func (i *appInstance) stopContainers(containersToStop int) error {
	...
	for c := 0; c < containersToStop && len(containerIds) > 0; c++ {
		...
		err := i.Scheduler.Stop(&types.StopContainerArgs{
			ContainerId: containerId,
		})
		if err != nil {
...
func (i *appInstance) stoppableContainers() ([]string, error) {
	...
	for _, container := range containers {
		...
		keepWarmVal, err := i.Rdb.Get(context.TODO(), Keys.appKeepWarmLock(i.Workspace.Name, i.Stub.ExternalId, container.ContainerId)).Int()
...
func (i *appInstance) genContainerId() string {
	return fmt.Sprintf("%s-%s-%s", appContainerPrefix, i.Stub.ExternalId, uuid.New().String()[:8])
}

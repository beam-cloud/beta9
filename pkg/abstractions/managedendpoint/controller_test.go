...
func TestDemandLeasesAreIsolatedIdempotentAndExpiring(t *testing.T) {
	...
	for _, id := range []string{"gateway-a/request-1", "gateway-a/request-1", "gateway-b/request-2"} {
		_, err := s.demand(ctx, "model", leaseAcquire, id, 0)
		require.NoError(t, err)
	}
	d, err := s.demand(ctx, "model", demandRead, "", 0)
	require.NoError(t, err)
	assert.EqualValues(t, 2, d.active)
	assert.True(t, d.warm)
	redis.SetTime(now.Add(40 * time.Second))
	_, err = s.demand(ctx, "model", leaseRenew, "gateway-b/request-2", 0)
	require.NoError(t, err)
	...
	_, err = s.demand(ctx, "model", leaseRenew, "gateway-a/request-1", 0)
	require.Error(t, err, "an expired owner must not resurrect itself")
	d, err = s.demand(ctx, "model", leaseRelease, "gateway-b/request-2", 0)
	require.NoError(t, err)
...
func TestDemandBoundsRequestsAcrossGateways(t *testing.T) {
	...
	for i := range serverlessAdmissionHeadroom {
		_, err := s.demand(ctx, "model", leaseAcquire, fmt.Sprint(i), 0)
		require.NoError(t, err)
	}
	_, err := s.demand(ctx, "model", leaseAcquire, "overflow", 0)
	require.ErrorIs(t, err, errDemandLimit)
	_, err = s.demand(ctx, "model", leaseRenew, "0", 0)
	require.NoError(t, err, "the cap must not interrupt existing work")
	_, err = s.demand(ctx, "model", leaseRelease, "0", 0)
	require.NoError(t, err)
	_, err = s.demand(ctx, "model", leaseAcquire, "overflow", 0)
	require.NoError(t, err)
}
...
func TestDemandAdmissionHeadroomPreservesScaleOut(t *testing.T) {
	...
	for i := 0; i < 128; i++ {
		_, err := s.demand(ctx, endpoint.Spec.ID, leaseAcquire, fmt.Sprint(i), 128)
		require.NoError(t, err)
	}
	...
	_, err = s.demand(ctx, endpoint.Spec.ID, leaseAcquire, "after-eviction", 0)
	require.ErrorIs(t, err, errDemandLimit)
	_, err = s.demand(ctx, endpoint.Spec.ID, leaseRenew, "request-129", 0)
	require.NoError(t, err, "shrinking capacity must not cancel an admitted request")
	_, err = s.demand(ctx, endpoint.Spec.ID, leaseRelease, "0", 0)
	require.NoError(t, err)
}
...
func TestDemandLimitsAdmissionHeadroomBeyondReadyCapacity(t *testing.T) {
	...
	for i := 0; i < readyCapacity+serverlessAdmissionHeadroom; i++ {
		_, err := s.demand(ctx, "model", leaseAcquire, fmt.Sprint(i), readyCapacity)
		require.NoError(t, err)
	}
	_, err := s.demand(ctx, "model", leaseAcquire, "overflow", readyCapacity)
...
func TestServerlessRolloutStartsReplacementOnSpareCapacity(t *testing.T) {
	...
	ctx := context.Background()
	_, err := s.demand(ctx, endpoint.Spec.ID, leaseAcquire, "request", 0)
	require.NoError(t, err)
...
func TestRejectedDemandCoalescesAndExpiresWithoutActiveLeases(t *testing.T) {
	...
	for i := range 20 {
		...
		require.NoError(t, err)
		_, err = s.demand(ctx, "model", leaseRelease, id, 0)
		require.NoError(t, err)
	}
	d, err := s.demand(ctx, "model", demandRead, "", 0)
	require.NoError(t, err)
	...
	redis.SetTime(now.Add(leaseTTL + time.Second))
	d, err = s.demand(ctx, "model", demandRead, "", 0)
	require.NoError(t, err)
	assert.Zero(t, d.active)

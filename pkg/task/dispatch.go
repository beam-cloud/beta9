			...
			for _, taskMessage := range tasks {
				...
				if !claimed {
					if time.Now().After(taskMessage.Policy.Expires) {
						...
						if err != nil {
							common.Logger.Errorf(ctx, "<dispatcher> unable to cancel task: %s, %v", task.Metadata().TaskId, err)
						}
...
func (d *Dispatcher) retryTask(ctx context.Context, task types.TaskInterface, taskMessage *types.TaskMessage) error {
	...
	if taskMessage.Retries >= taskMessage.Policy.MaxRetries {
		...
		if taskMessage.Policy.MaxRetries != 0 {
			common.Logger.Infof(ctx, "<dispatcher> hit retry limit, not reinserting task <%s> into queue: %s", taskMessage.TaskId, taskMessage.StubId)
		}

		err = task.Cancel(ctx, types.TaskExceededRetryLimit)
		if err != nil {
			common.Logger.Infof(ctx, "<dispatcher> unable to cancel task: %s, %v", task.Metadata().TaskId, err)
			return err
		}
	...
	if err != nil {
		common.Logger.Infof(ctx, "<dispatcher> failed to remove task claim: %s, %v", task.Metadata().TaskId, err)
		return err
	}

	// Retry task
	common.Logger.Infof(ctx, "<dispatcher> missing heartbeat, reinserting task<%s:%s> into queue: %s",
	...
	err = task.Retry(ctx)
	if err != nil {
		common.Logger.Errorf(ctx, "<dispatcher> retry failed: %+v", err)
		return err
	}

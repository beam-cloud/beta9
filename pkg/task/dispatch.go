			...
			for _, taskMessage := range tasks {
				...
				if !claimed {
					if time.Now().After(taskMessage.Policy.Expires) {
						...
						if err != nil {
							common.Logger.Errorf("<dispatcher> unable to cancel task: %s, %v", task.Metadata().TaskId, err)
						}
...
func (d *Dispatcher) retryTask(ctx context.Context, task types.TaskInterface, taskMessage *types.TaskMessage) error {
	...
	if taskMessage.Retries >= taskMessage.Policy.MaxRetries {
		...
		if taskMessage.Policy.MaxRetries != 0 {
			common.Logger.Infof("<dispatcher> hit retry limit, not reinserting task <%s> into queue: %s", taskMessage.TaskId, taskMessage.StubId)
		}

		err = task.Cancel(ctx, types.TaskExceededRetryLimit)
		if err != nil {
			common.Logger.Infof("<dispatcher> unable to cancel task: %s, %v", task.Metadata().TaskId, err)
			return err
		}
	...
	if err != nil {
		common.Logger.Infof("<dispatcher> failed to remove task claim: %s, %v", task.Metadata().TaskId, err)
		return err
	}
	...
	err = task.Retry(ctx)
	if err != nil {
		common.Logger.Errorf("<dispatcher> retry failed: %+v", err)
		return err
	}

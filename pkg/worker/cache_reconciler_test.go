...
func (*fakeEventRepo) HasDurableScopedStateSink() bool { return true }
...
func TestPressureProtectedContentPrioritizesNewestStubWorkingSet(t *testing.T) {
	...
	stubs := []recentStubContent{
		{
			...
			items: []types.CacheRequiredContentItem{
				...
				{Hash: "new-state-manifest", Kind: types.CacheContentKindStateManifest, SizeBytes: 40},
			},
		},
		{
			stub: cache.RecentStub{WorkspaceID: "ws", StubID: "old", LastSeen: now.Add(-time.Hour)},
			items: []types.CacheRequiredContentItem{
				{Hash: "old-state-manifest", Kind: types.CacheContentKindStateManifest, SizeBytes: 40},
	...
	require.Contains(t, protected, "new-state-manifest")
	require.Contains(t, protected, "new-volume")
	require.NotContains(t, protected, "old-state-manifest")
	require.NotContains(t, protected, "old-volume")
}

func TestPressureProtectedContentPrioritizesStateManifestWithinStub(t *testing.T) {
	now := time.Now()
	stubs := []recentStubContent{
		{
			...
			items: []types.CacheRequiredContentItem{
				...
				{Hash: "state-manifest", Kind: types.CacheContentKindStateManifest, SizeBytes: 50},
			},
		},
	}
	...
	require.Contains(t, protected, "state-manifest")
...
func TestReconcileStubSkipsRecentlyMaterializedMissingContent(t *testing.T) {
	...
	manager.reconcileStub(localServer, localHost.HostId, cache.RecentStub{WorkspaceID: "workspace", StubID: "stub"}, nil)
...
func testClipV1Metadata(t *testing.T) *clipCommon.ClipArchiveMetadata {
	...
	require.NoError(t, archiver.Create(clip.ClipArchiverOptions{
		SourcePath: src, OutputFile: archivePath, ArchivePath: archivePath,
	}))

...
func TestInjectNvidiaEnvVarsExistingCudaInImage(t *testing.T) {
	manager := NewContainerNvidiaManager(4)
	initialEnv := []string{"INITIAL=1", "NVIDIA_REQUIRE_CUDA=", "CUDA_VERSION=11.8.2"}
	...
	expectedEnv := []string{
		...
		"NVIDIA_REQUIRE_CUDA=",
		"CUDA_VERSION=11.8.2",
		"PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/usr/local/cuda-11.8/bin",
		"LD_LIBRARY_PATH=/usr/lib/x86_64-linux-gnu:/usr/lib/worker/x86_64-linux-gnu:/usr/local/nvidia/lib64:/usr/local/cuda-11.8/targets/x86_64-linux/lib",
	}

		...
		case <-ticker.C:
			...
			if err != nil {
				if _, ok := err.(*types.ProviderNotImplemented); ok {
					...
					common.Logger.Infof(ctx, "<provider %s>: unable to list machines - %v", p.Name, err)
					continue
				}
			}
			...
			for machineId, instanceId := range machines {
				func() {
					...
					if err != nil {
						common.Logger.Infof(ctx, "<provider %s>: unable to retrieve machine <machineId: %s> - %v", p.Name, machineId, err)
					...
					if err != nil {
						common.Logger.Infof(ctx, "<provider %s>: unable to retrieve workers for machine <machineId: %s> - %v", p.Name, machineId, err)
						return
					}

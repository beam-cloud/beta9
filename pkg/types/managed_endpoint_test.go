...
func TestSpecValidate(t *testing.T) {
	...
	assert.Equal(t, "acme/model", model.ID)
	...
	for _, id := range []string{"", "acme/", "/model", "acme/model/extra", "acme model", "Acme/Model!"} {
		...
		spec.ID = id
		spec.Normalize()
		assert.Error(t, spec.Validate(), id)
	}
}
...
// Price is exact and itemized: the total is rounded once and the components
// are reconciled to it; invalid or overflowing work is refused.
...
func TestChargeSettlesOnceFromReportedWork(t *testing.T) {
	...
	flat := Charge{ID: "gen-flat", Pricing: Pricing{Request: "0.05"}}
	require.NoError(t, flat.Settle(Work{PromptTokens: 123}, now))
	require.Equal(t, Work{Requests: 1}, flat.Work, "flat work is one request regardless of what the app reported")
	require.EqualValues(t, 50_000, flat.Cost.MicroUSD)

	failed := Charge{ID: "gen-2", Pricing: Pricing{Request: "0.05"}}

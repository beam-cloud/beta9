...
func (cs *Server) serveRawRead(conn net.Conn, req rawReadRequest) {
	...
	if err == nil && ok {
		...
		for _, region := range regions {
			...
			file, cachedFD, err := cs.openRawReadPage(region.path, cacheableFD)
			if err != nil {
				status = "open_error"
			...
			reader := io.NewSectionReader(file, copyOffset, copyLength)
...
func (cs *Server) openRawReadPage(path string, cacheable bool) (*os.File, bool, error) {
	if cacheable && cs != nil && cs.rawPageFDCache != nil {
		file, cacheHit, err := cs.rawPageFDCache.get(path)
		if err == nil {
			if cacheHit {

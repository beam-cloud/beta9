...
func (c *ConcurrencyLimitGroup) GetConcurrencyLimitByWorkspaceId(ctx echo.Context) error {
	...
	if workspace.ConcurrencyLimitId == nil {
		return NewHTTPError(http.StatusNotFound, "Concurrency limit not found")
	}

	concurrencyLimit, err := c.backendRepo.GetConcurrencyLimit(ctx.Request().Context(), *workspace.ConcurrencyLimitId)
	if err != nil {
		return NewHTTPError(http.StatusInternalServerError, "Failed to get concurrency limit")
	}
...
func (c *ConcurrencyLimitGroup) CreateOrUpdateConcurrencyLimit(ctx echo.Context) error {
	...
	if workspace.ConcurrencyLimitId != nil {
		...
		if err != nil {
			return NewHTTPError(http.StatusInternalServerError, "Failed to update concurrency limit")
		}

		err = c.workspaceRepo.SetConcurrencyLimitByWorkspaceId(workspaceId, concurrencyLimit)
		if err != nil {
			return NewHTTPError(http.StatusInternalServerError, "Failed to recache concurrency limit")
		}
	...
	if err != nil {
		return NewHTTPError(http.StatusInternalServerError, "Failed to create concurrency limit")
	}

	err = c.workspaceRepo.SetConcurrencyLimitByWorkspaceId(workspaceId, concurrencyLimit)
	if err != nil {
		return ctx.JSON(http.StatusInternalServerError, "Failed to recache concurrency limit")
	}
...
func (c *ConcurrencyLimitGroup) DeleteConcurrencyLimit(ctx echo.Context) error {
	...
	if err != nil {
		return ctx.JSON(http.StatusInternalServerError, "Failed to delete concurrency limit")
	}
